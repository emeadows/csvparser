...
class FileProcessingSpec extends AnyWordSpec with Matchers {
  ...
  "rejoinStringsWhereNeeded" should {
    ...
    "handle data till end of actual line" in {
      ...
      val emptyResultData = ResultData(Nil, None, Nil, balanced = true)
      ...
      val resultData1     = ResultData(Nil, Some(updatedLine1), Nil, balanced = false)
      ...
      val resultData2    = ResultData(Nil, Some(updatedLine2), Nil, balanced = false)
      ...
      val resultData3    = ResultData(Nil, Some(updatedLine3), Nil, balanced = false)
      ...
      val resultData4    = ResultData(Nil, Some(updatedLine4), Nil, balanced = false)
      ...
      val data =
        List(
          ...
          "\"\nsomething with \nmany line breaks\n\"",
          "more things",
          "\"something plane in quote\""
        )
      val fullLineResult = ResultData(Nil, None, data, balanced = true)
      fileProcessing.rejoinStringsWhereNeeded(resultData4, splitByQuotes5) shouldBe fullLineResult
    }

  }

  "updateAsGoodData" should {
    "return string to be parsed" in {
      val restult = fileProcessing.updateAsGoodData(
        Some("something,\"something with a line break\n\",\"\nsomething with \nmany line breaks\n"),
        ",more things,\"something plane in quote\""
      )
      restult shouldBe "something,\"something with a line break\n\",\"\nsomething with \nmany line breaks\n\",more things,\"something plane in quote\""
    }
  }

  "processInputStringWithDelimiter" should {
    "return expected list" in {
      val input =
        "something,\"something with a line break\n\",\"\nsomething with \nmany line breaks\n\",more things,\"something plane in quote\""
      fileProcessing.processInputStringWithDelimiter(input).toOption.get shouldBe List(
        "something",
        "\"something with a line break\n\"",
        "\"\nsomething with \nmany line breaks\n\"",
        "more things",
